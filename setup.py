...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_structured_text", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_structured_text": ["*.pyi", "py.typed"],
        "tree_sitter_structured_text.queries": ["*.scm"],
    },
    ext_package="tree_sitter_structured_text",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
